...
<Steps>
  <Step title="Generate an API key" icon="key-round">
    In the Ringg dashboard, open **Settings → API Key** and click **Generate API Key**. The key is shown **only once**, so store it in a secret manager straight away - it cannot be retrieved afterwards, and losing it means regenerating, which immediately revokes the old key and breaks any integration still using it. Send it on every API request as `X-API-KEY`.
  </Step>
