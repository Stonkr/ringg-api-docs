...
| `data-ringg` | Element |
|---|---|
| `header` | In-call header band. **Only present during a call**, which integrators use to detect an active call (e.g. `:has([data-ringg="header"])`). Absolute-positioned frosted overlay. |
...
| `header-status-dot` | Presence dot. In-call only. Carries `data-ringg-connecting`: muted and pulsing while connecting, success-colored once live. |
...
The system log pill renders the inline activity feed in the transcript. It only shows up when you opt in via `loadAgent({ eventLogs: { enabled: true } })`. See [Agent actions](/get-started/guides/embedding-widget-components-data#agent-actions) for details.
...
| `message-input` | Outer pill container carrying the frosted-glass treatment; transparent gutters. Also wraps the slash menu when open (going solid). |
...
Block trees the agent streams on the `ringg.blocks` topic: the cards, forms and pickers built under **Embed & Widgets → Components** (see [Components](/get-started/guides/embedding-widget-components)). Rendered full-width in the transcript.
