...
Whitelist the Ringg SIP addresses on your SIP gateway for the following:
...
Ringg provides its SIP signalling and RTP media IP addresses during onboarding. Share your source IPs with the Ringg team and we will whitelist them on our side.
...
sequenceDiagram
    ...
    participant R as Ringg sip.ringg.ai
...
Ringg's outbound API calls come from a fixed set of source IPs. Ringg shares these during onboarding, and they must be whitelisted on your firewall.
...
2. The Ameyo PBX sends BYE to the Ringg SIP endpoint on port 5060 to tear down the bot call leg.
...
<Steps>
  <Step title="SIP Gateway IPs">
    Share the source IP(s) of your SIP gateway so we can whitelist them on the Ringg SIP ingress.
  </Step>
