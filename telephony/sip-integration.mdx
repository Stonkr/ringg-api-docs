...
`sip.ringg.ai` resolves to `35.227.95.16`. Use the FQDN as the destination in your trunk configuration, and use the IP for firewall and ACL rules.
...
<Warning>
The most common cause of a connected call with no audio is a firewall that permits SIP on 5060 but blocks the RTP range. Allow UDP `10000-60000` in both directions to `35.227.95.16` before testing.
</Warning>
...
For outbound calls, Ringg originates the INVITE toward your platform from `35.227.95.16`. Allow that address in your inbound firewall and SIP ACL.
...
<AccordionGroup>
  <Accordion title="IP based (simplest)">
    You allowlist `35.227.95.16` on your side and no credentials are exchanged. Recommended where your platform supports it.
  </Accordion>
...
| Call connects but there is no audio | RTP range blocked | Allow UDP `10000-60000` to and from `35.227.95.16` |
...
<Steps>
  <Step title="Contact Ringg to start the integration">
    ...
    Ringg confirms the allowlist entry and returns your trunk configuration.
  </Step>
  <Step title="Apply your firewall rules">
    Point your trunk at `sip.ringg.ai`, and allow `35.227.95.16` on 5060 UDP and TCP, 5061 TLS, plus UDP `10000-60000` for RTP, in both directions.
  </Step>
